<h1 align="center">ACP Design: a alternativa open-source ao Claude Design</h1>

> ⚡ **[ACP Design Cloud — o serviço oficial de modelos.](https://open-design.ai/zh/pricing/)** Uma única recarga permite usar modelos de agentes e de imagem dentro do ACP Design: GPT, Claude e DeepSeek para agentes; GPT Image 2.0, Seedream 5.0 Pro e Nano Banana 2.0 para imagens.
>
> 🚀 **[DeepSeek V4 Flash e V4 Pro já estão disponíveis.](https://open-design.ai/zh/pricing/)** Use inteligência de ponta em protótipos, apresentações, sistemas de design e tarefas cotidianas de agentes. Membros do ACP Design podem usar os dois modelos sem limites por duas semanas, diretamente no aplicativo.
>
> 🧩 **[DeepSeek Harness agora é compatível.](https://open-design.ai/zh/agents/deepseek-harness-design/)** Conecte o Agent Harness oficial `dsh` da DeepSeek ao ACP Design como runtime nativo, com raciocínio estruturado, chamadas de ferramentas, descoberta de modelos, cancelamento e retomada de sessão. Os arquivos gerados permanecem no fluxo do ACP Design para visualização ao vivo e entrega.
...
<p align="center">
  ...
  <a href="https://open-design.ai/cloud/">ACP Design Cloud</a> ·
...
O ACP Design transforma esse loop em um **sistema de arquivos de skills funcionais, templates de renderização, design systems e plugins** que seus agentes conseguem ler, escrever e remixar.
...
> O ACP Design é entregue como **skills, uma CLI e um servidor MCP** que os coding agents populares consomem nativamente. Depois que o OD está instalado, um único `od mcp install <agent>` conecta o servidor MCP à configuração daquele agente, e você chama as mesmas ferramentas de dentro de qualquer agente.
...
<p align="center">
  <img src="https://repo-assets.open-design.ai/resources/images/coding-agents.png" alt="The 25 coding-agent CLIs ACP Design supports — Claude Code · Codex · OpenCode · Hermes · Antigravity · Vela · Grok Build · Kimi · Cursor Agent · Qwen · Qoder · GitHub Copilot · Pi · Kiro · Kilo · Mistral Vibe · DeepSeek · Reasonix · Aider · Amp · CodeBuddy · Mimo · AtomCode · Devin · Trae" width="100%" />
</p>
...
O **[HyperFrames][hyperframes]** é o framework de vídeo open-source e agent-native da HeyGen, integrado como cidadão de primeira classe no ACP Design. O agente escreve HTML + CSS + GSAP, e o HyperFrames renderiza tudo para um MP4 determinístico via Chrome headless + FFmpeg. Combine-o com o **Seedance 2.0** para t2v / i2v cinematográfico, **Veo 3 / Sora 2 / Kling 2** para variantes de modelo roteadas e **Suno v5 / Lyria 2** para a camada de áudio.
...
O ACP Design (OD) é a alternativa open-source. Mesmo loop, mesmo modelo mental artifact-first, nada do lock-in:
...
A forma mais rápida de usar o ACP Design. Sem Node, sem pnpm, sem clone.
...
Você pode usar o ACP Design sem nunca abrir a GUI — chame-o como skill, plugin ou servidor MCP dentro do Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi e mais.
...
Em uma execução CLI local com filesystem, o agente combina a skill funcional ou o template escolhido com seu `DESIGN.md`, grava os arquivos canônicos e o ACP Design os mostra no preview. Uma execução BYOK/API sem ferramentas de arquivos retorna um único bloco `<artifact>` completo.
...
O template da Sealos App Store executa a imagem Docker publicada do ACP Design com armazenamento persistente para o workspace e Basic Auth no proxy público. Para implantações Docker públicas ou compartilhadas personalizadas, siga as orientações de proxy reverso e `OPEN_DESIGN_ALLOWED_ORIGINS` em [`deploy/README.md`](../../deploy/README.md#local-compose).
...
5. **O ACP Design fica mais inteligente conforme você usa.** Seus screenshots, fontes, paletas e artefatos confirmados se acumulam como padrões para a próxima sessão. Menos retrabalho, menos desvio.

---

## Use o ACP Design a partir do seu coding agent

O ACP Design entrega um **servidor MCP stdio** e **scripts de instalação** por agente. Qualquer agente compatível com MCP em outro repositório pode ler arquivos diretamente dos seus projetos locais do ACP Design — tokens CSS, componentes JSX, HTML de entrada — como uma API estruturada consultável por nome. O agente sempre vê o arquivo ao vivo, não uma exportação desatualizada.
...
Um plugin do ACP Design exige `open-design.json` e o payload do tipo. Skills e cenários também incluem `SKILL.md`; outros tipos usam seu payload próprio:
...
1. Solte a pasta do plugin em [`plugins/community/`](../../plugins/community/) (plugins de terceiros), ou — para entregá-lo empacotado com o ACP Design — no tier correspondente de [`plugins/_official/`](../../plugins/_official/).
...
- [x] **0.9.0** — ACP Design Cloud (Model Router oficial embutido no app: zero configuração, login de um clique)
...
- 💬 **Discord** — chat diário, compartilhamento de plugins, perguntas → [**discord.gg/jE4MzArHX**](https://discord.gg/jE4MzArHX)
...
O ACP Design continua avançando porque os contribuidores — designers, engenheiros, autores de prompts — continuam aparecendo. Muitas das skills, design systems e plugins mais usados foram escritos por pessoas fora do time central.
...
### 🏅 Programa ACP Design Fellow

Estamos recrutando **ACP Design Fellows** ao redor do mundo — os Fellows moldam o produto ao lado do time central, representam oficialmente o ACP Design em sua região e fazem a comunidade crescer localmente, com suporte financiado ($1,000 / MR), créditos gratuitos de LLM e uma trilha de revisão direta. Detalhes → [`MAINTAINERS.md`](../../MAINTAINERS.md) e o anúncio no [Discord](https://discord.gg/jE4MzArHX).
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | O framework de motion graphics HTML→MP4, integrado como o `hyperframes-html` de primeira classe no ACP Design. |
