...
Agent-facing guidance for the ACP Design curated integration of Cloudflare Kumo UI.
...
- Use Kumo's native `data-mode="dark"` hook; `data-theme="dark"` is included only for ACP Design artifact compatibility.
- Treat the section, container, and extended heading tokens as ACP Design compatibility bindings, not upstream Kumo tokens.
