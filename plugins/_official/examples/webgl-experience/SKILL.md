...
description: |
  ...
  ACP Design serves this in powered-preview mode so the GPU stack actually
  runs.
triggers:
  - "webgl"
  - "webgl2"
...
ACP Design detects `getContext('webgl2')` / `new Worker` / `SharedArrayBuffer` and renders this file in **powered preview** — a cross-origin-isolated iframe with `allow-same-origin`. That means real Web Workers, `SharedArrayBuffer`, WASM, and the full GPU pipeline are available. You do **not** need to work around the opaque sandbox. Prefer `webgl2` for modern features; fall back to `webgl` when the effect allows.
