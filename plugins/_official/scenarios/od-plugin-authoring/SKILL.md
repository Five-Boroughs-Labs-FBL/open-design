...
description: Guided scenario for creating an ACP Design plugin folder that can be installed into My plugins.
od:
...
Use this scenario when the user wants to create their own ACP Design plugin.
...
- Treat `SKILL.md` as the canonical behavior description. `open-design.json` should describe how ACP Design installs, applies, and presents that behavior.
...
- Include a short readiness summary when finished:
  ...
  - A direct next-action prompt that offers: Add to My plugins, Publish repo, or ACP Design PR.
