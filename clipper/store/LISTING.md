# ACP Design Web Clipper — Store Listing & ASO Pack
...
- **Public name:** ACP Design Web Clipper
- **Version:** 0.2.0 (MV3)
...
| Field | Value |
| --- | --- |
| Store title | `ACP Design Web Clipper` |
| Toolbar / action title | `ACP Design Web Clipper` |
| Short brand | ACP Design |
| Tagline | Clip the web into your design Library. |
| Accent color | Ember `#c96442` |
| Brand ink | `#202020` |

Why this name: leads with the **ACP Design** brand and pairs it with the exact
...
### Title (max 75)
```
ACP Design Web Clipper
```

### Summary / short description (max 132)
```
Clip pages, design systems, screenshots, images, and Figma import JSON into your ACP Design Library — local-first, one click, no login.
```

### Detailed description (max 16,000)
```
ACP Design Web Clipper saves anything you find on the web straight into your ACP Design Library — the local asset registry of the open-source ACP Design app. One click, no account, no pairing, no tokens.
...
• Page → Figma JSON
  Every page capture also computes an importable Figma node-tree (frames, text, images, fills, strokes, corner radii, shadows) you can rebuild as editable layers with the companion ACP Design Figma plugin. This JSON is imported from inside the plugin, not by dragging it into Figma Drafts.

• Right-click any image → "Save image to ACP Design Library".
...
• Local-first & private. Everything you clip goes only to the ACP Design app running on your own machine (loopback 127.0.0.1). Nothing is sent to us. No analytics, no tracking, no ads.

• Zero setup. No sign-in, no API keys, no pairing codes. The popup shows "● Connected" the moment it detects ACP Design running, and you can clip right away.
...
1. Install the open-source ACP Design app — https://open-design.ai
2. Start it so the local daemon is running.
3. Click the ACP Design Web Clipper toolbar icon and capture. That's it.

ACP Design is the local-first, open-source alternative to Claude Design and a Figma alternative for the agent era. The Web Clipper is how the web gets into it.
...
```
Capture web content — full pages, design systems, screenshots, images, page elements, and Figma import JSON — and save it into the user's locally running ACP Design app Library.
```

### Permission justifications (one per requested permission)
| Permission | Justification to paste |
| --- | --- |
| `host_permissions: <all_urls>` | The user can clip from any website they choose. We need to read and screenshot the active page, and fetch + inline its cross-origin resources (CSS, images) to produce a complete, self-contained snapshot. The same host access reaches the user's local ACP Design daemon at 127.0.0.1. Standard for web clippers. |
...
| `contextMenus` | Provide the right-click "Save image to ACP Design Library" menu entry. |
...
- **Does this item collect or use user data?** It transmits captured page content **only to the user's own local ACP Design app** (loopback) on explicit user action. It does **not** send any data to the developer or any third party.
...
2. **The ACP Design Library filling up** — `Everything lands in your Library, live, with a Clipper badge.`
...
- **Name:** `ACP Design Web Clipper`
- **Summary (≤250):** `Clip web pages, design systems, screenshots, images, and Figma import JSON straight into your ACP Design Library. Local-first, one click — no account, no pairing, no tokens.`
...
core promise: captured content goes only to the user's local ACP Design app;
...
- **名称:** `ACP Design Web Clipper`（品牌名保持英文；本地化副标题：网页剪藏）
- **简短描述 (≤132):**
```
一键将任意网页、设计系统、截图、图片或 Figma 导入 JSON 剪藏到本地的 ACP Design 素材库 —— 本地优先，无需登录。
```
- **详细描述:**
```
ACP Design Web Clipper 把你在网上看到的任何内容一键剪藏进 ACP Design 素材库（开源 ACP Design 应用的本地资源库）。无需账号、无需配对、无需令牌。
...
• 页面 → Figma：每次整页捕获都会生成可导入的 Figma 节点树，用 ACP Design Figma 插件还原为可编辑图层。
• 右键图片 →「保存到 ACP Design 素材库」。
• 可选页面工具条：默认隐藏，截图时自动移出画面。

【为何不同】
• 本地优先、隐私安全：所有内容只发送到你本机运行的 ACP Design 应用（127.0.0.1），我们不收集任何数据，无统计、无追踪、无广告。
• 零配置：无登录、无密钥、无配对码。检测到 ACP Design 运行即显示「● 已连接」。
