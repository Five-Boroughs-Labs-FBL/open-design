...
description: |
  ...
  ACP Design browser-tab context, testing web apps, dogfooding ACP Design
  previews, QA, bug hunts, or reviewing app quality. Prefer local ACP Design
...
Use `agent-browser` for local ACP Design preview validation: inspect rendered
...
For selected ACP Design browser tabs and browser-use/browser-harness-style
...
5. If the selected ACP Design context only provided a URL/title and no browser
...
Do not run ACP Design's own daemon CLI as a browser automation tool. Commands
...
With the ACP Design preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `ACP Design`, current URL under `127.0.0.1:17573`,
visible ACP Design UI text in the snapshot, and a screenshot at
