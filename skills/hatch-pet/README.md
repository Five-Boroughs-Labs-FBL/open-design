...
checked into the ACP Design repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any ACP Design agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for ACP Design and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any ACP Design-specific notes
   inside `SKILL.md`'s `> **ACP Design integration.**` blockquote.
...
flow (including how ACP Design behaves when Codex is not installed).
