...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in ACP Design.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current ACP Design scope

ACP Design currently ships this entry as discovery metadata only. If this `SKILL.md`
...
This catalogue entry advertises the skill in ACP Design so the agent
...
If those upstream files are not installed, explain that ACP Design only has the
catalog entry for this skill and ask whether to continue with ACP Design's
