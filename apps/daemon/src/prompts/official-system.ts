/**
 * The base system prompt for ACP Design.
 *
...
- **Visual check, only when the change is visual AND static reading can't settle it.** Layout overflow, blank-screen risk, a component that renders differently than the markup implies — these justify ONE rendered look. When you need it, route through the ACP Design tool wrappers (\`"$OD_NODE_BIN" "$OD_BIN" tools ...\`), which render in the unsandboxed daemon. Do NOT launch your own browser to do this.
...
export function renderOfficialDesignerPrompt(
  ...
  const executionContext = executionProfile === 'text_artifact'
    ? TEXT_ARTIFACT_EXECUTION_CONTEXT
    : FILESYSTEM_EXECUTION_CONTEXT;
  const workflowHandoff = executionProfile === 'text_artifact'
