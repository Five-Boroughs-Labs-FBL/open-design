...
const TEXT_ARTIFACT_MULTI_SURFACE_WORKFLOW_HANDOFF = `4. **Build the primary artifact.** Stream one complete HTML document for the first claimed surface as \`<artifact identifier="<surfaceId>" type="text/html">\`, then close \`</artifact>\` before planning or spawning. Do not put thinking or other-screen write-ups in that file. You may Write a complete primary file; stop streaming HTML into it afterwards.
5. **Cover the rest.** After the primary is a complete document you HAVE filesystem tools for remaining claimed surfaces only. Spawn one general-purpose sub-agent per remaining claimed surface, or Write those exact declared files. Each artifact identifier MUST equal its surface id. Do not emit unclaimed HTML. Do not stop after the primary screen.

## Text-artifact handoff
Grok CLI live-paints from \`<artifact type="text/html">\` blocks. The live primary must stay one complete HTML document. Remaining claimed surfaces may use filesystem Write on their exact declared files. Identifier = surface id. Extra unclaimed HTML is forbidden.`;
