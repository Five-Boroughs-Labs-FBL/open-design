...
- \`update-app\` -- "ACP Design needs an update before it can generate images."
  Simplified Chinese, exactly:
  需要更新 ACP Design 才能生成图片。
...
- \`contact-support\` -- "The image didn't come out, and it isn't anything you
  did. This one is on ACP Design and we've logged it; trying again usually
  recovers, and if it keeps happening, contact us." Simplified Chinese,
  exactly:
  图片没生成出来,不是你的操作有误 —— 这次是 ACP Design 自己的问题,我们已经记下了。重试一般能恢复;反复出现的话联系我们。
...
ACP Design-owned media execution is **disabled for this run**. Do not call
...
ACP Design Cloud image and video models use the \`vela/*\` catalogue prefix.
...
**Default recipe — use ACP Design's scaffold, don't write from scratch.**
...
# ACP Design writes the required files itself; HyperFrames init is never run.
