...
function renderUiLocalePrompt(
  ...
  const lines = [
    '# UI locale override',
    '',
    `The ACP Design UI locale for this run is \`${normalized}\` (${languageName}). All user-visible chat prose and generated UI controls must follow this locale, especially \`<question-form>\` titles, question labels, placeholders, and option labels. Keep machine-readable ids and object option \`value\` fields exact and unlocalized.`,
...
ACP Design Cloud models use the \`vela/*\` prefix. Never invoke the \`vela\`
...
### ACP Design Cloud media defaults
...
This run uses ACP Design's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
// to a "writable roots" setting ACP Design does not have. The user searched
...
ACP Design does not currently expose a sandbox mode, a writable-directory or "writable roots" list, or an approval-policy setting. Do not tell the user to look for one, and do not invent a settings path, menu, or option name to explain the failure — they will go looking and find nothing. Describe the limitation plainly and point at the Skills tab instead. (If a future build does add such a surface, this paragraph is what should change.)`;
...
This conversation is in ACP Design Ask mode: a fast, low-overhead chat kept deliberately light to save tokens. ACP Design is the open-source Claude Design alternative and a native Figma counterpart. Official links: GitHub https://github.com/nexu-io/open-design, website https://open-design.ai/, Discord https://discord.gg/jE4MzArHX.
...
For mid-conversation clarification you may still emit a \`<question-form>\` block — it is markup the ACP Design UI parses, not a native tool call.`;

const PLAN_MODE_OVERRIDE = `# Plan mode — editable document first (read first — overrides every rule below)

This conversation is in ACP Design Plan mode. Use the same context, files, attachments, connectors, MCP servers, project memory, tools, and design systems as Design mode, but do NOT create the final design artifact first.
...
function renderMediaMetadataAction(
  ...
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but ACP Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
