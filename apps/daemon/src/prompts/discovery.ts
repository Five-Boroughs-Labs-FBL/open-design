...
Stream each claimed surface as \`<artifact identifier="<surfaceId>" type="text/html">\`. The live primary must be exactly one complete HTML document: no thinking, no other-screen write-ups. Close \`</artifact>\` before planning or spawning. After the primary is complete you HAVE filesystem tools for remaining claimed surfaces only. Identifier must equal the surface id. Do not emit unclaimed HTML. Do not stop after the first screen.
