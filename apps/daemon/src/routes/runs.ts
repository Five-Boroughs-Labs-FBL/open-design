...
export function registerRunRoutes(app: Express, ctx: RegisterRunRoutesDeps) {
  ...
  const handleRunCreate = async (req: ApiRequest, res: ApiResponse) => {
      ...
      if (!explicitExecutablePlugin) {
        ...
        if (strategyRolloutDecision?.effectiveMode === 'active') {
          ...
          const fallbackPluginId = selectedExamplePlugin?.id
            ?? (verifiedExampleBinding || requestBody.skipDefaultScenario === true ? null : defaultPluginId);
          if (
            fallbackPluginId
    ...
    let releaseWriterReservation: () => void = () => undefined;
    try {
    ...
    if (resumed && resumeDesignGeneration && runProject) {
      try {
    ...
    const attemptedDesignGeneration = resumed
      ...
      : (preparedRun.kind === 'ready' && preparedRun.creationKind === 'created')
        ? preparedDesignGeneration
        : null;
  ...
  app.post('/api/chat', async (req: ApiRequest, res: ApiResponse) => {
    ...
    let releaseChatWriterReservation: () => void = () => undefined;
    try {
      releaseChatWriterReservation = await reserveProjectWriter(
        chatProject,
        preparedDesignGeneration !== null,
        existingLogicalRequest,
      );
