...
export function renderOAuthResultPage(opts: OAuthResultPageOptions): string {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to ACP Design.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — ACP Design</title>
