...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `ACP Design CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall ACP Design or Node and restart the daemon.`,
    );
  }
