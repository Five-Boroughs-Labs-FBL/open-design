...
export function buildVersionDiagnostic(
  ...
  if (!version) {
    return {
      ...
      severity: 'error',
      message: `${def.name} was found, but ACP Design could not verify its version.`,
  ...
  return {
    ...
    severity: 'warning',
    message: `${def.name} ${version} has not been tested with this ACP Design build.`,
...
export function buildCompatibilityDiagnostic(
  ...
  return {
    ...
    severity: 'error',
    message: `${def.name} is installed, but its ACP Design profile is missing or incompatible.`,
    detail: 'Install the pinned ACP Design profile bundle in Harness profile `open-design`, then rescan.',
    fixActions: [
      { kind: 'openDocs' },
      { kind: 'rescan' },
    ],
