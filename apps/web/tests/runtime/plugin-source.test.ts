...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the ACP Design repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'ACP Design', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('ACP Design');
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('ACP Design')).toBe('OD');
