...
describe('WhatsNewPopup version resolution', () => {
  ...
  it('never paints the placeholder while /api/version is still in flight', async () => {
    ...
    expect(screen.getByText(`ACP Design ${SHOW_PAYLOAD.version} is here`)).toBeTruthy();
  });
  ...
  it('switches to the running version once /api/version resolves', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`ACP Design ${RUNNING_APP_VERSION} is here`)).toBeTruthy();
    });
    expect(screen.queryByText(`ACP Design ${SHOW_PAYLOAD.version} is here`)).toBeNull();
  });
