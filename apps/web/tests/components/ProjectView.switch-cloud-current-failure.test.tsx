...
describe('OPEND-3205 new send owns the current failure presentation', () => {
  ...
  it.each([
    ...
    await act(async () => {
      fireEvent.click(within(oldCard).getByRole('button', { name: '切换到 ACP Design Cloud' }));
    });
    ...
    if (balanceBlocked) {
      ...
      expect(within(currentCard).queryByRole('button', { name: '切换到 ACP Design Cloud' })).toBeNull();
  ...
  it('keeps consumption across a same-tab remount but renders a genuinely new failed attempt', async () => {
    ...
    expect(within(current).queryByRole('button', { name: '切换到 ACP Design Cloud' })).toBeNull();
  ...
  it('does not consume another conversation failure when an old preflight finishes after navigation', async () => {
    ...
    try {
      ...
      expect(within(secondCard).getByRole('button', { name: '切换到 ACP Design Cloud' })).toBeVisible();
  ...
  it('does not consume a side conversation failure when a non-composer board task is queued in the primary conversation', async () => {
    ...
    expect(within(sideCard).getByRole('button', { name: '切换到 ACP Design Cloud' })).toBeVisible();
