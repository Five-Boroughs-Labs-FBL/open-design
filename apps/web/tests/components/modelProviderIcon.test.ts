...
import { modelProviderIcon, modelProviderIconSrc } from '../../src/components/modelProviderIcon';
...
describe('modelProviderIconSrc', () => {
  it('maps AMR-style provider/model ids to their bundled brand mark', () => {
    ...
    expect(modelProviderIconSrc('xai/grok-4')).toBe('/agent-icons/grok-build.svg');
  ...
  it('matches on provider-name substrings so aliased prefixes still resolve', () => {
    ...
    expect(modelProviderIconSrc('grok/grok-4')).toBe('/agent-icons/grok-build.svg');
    expect(modelProviderIconSrc('grok-4.6')).toBe('/agent-icons/grok-build.svg');
  });

  it('renders grok through the mono AgentIcon path so dark theme stays legible', () => {
    expect(modelProviderIcon('grok-4.6')).toEqual({ kind: 'agent', id: 'grok-build' });
    expect(modelProviderIcon('xai/grok-4')).toEqual({ kind: 'agent', id: 'grok-build' });
  });
