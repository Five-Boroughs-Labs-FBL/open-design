...
/**
 * 报错卡**标题**那一行的三格:S01 / S02 / S04(《ACP Design 报错文案｜精简版》)。
 *
 ...
 *      S04(ACP Design 智能体没授权,新稿「ACP Design 尚未登录」)今天共用
...
describe('报错卡标题 · S01 / S02 / S04', () => {
  ...
  it('S04:ACP Design 智能体没授权 —— 标题说的是 ACP Design 自己', () => {
  ...
  it('S02 和 S04 不是同一句话 —— 一个键装不下两格', () => {
     ...
     * AMR 的 agent 名恰好是 `ACP Design`,所以哪怕两格共用一个键,渲染出来
     * 也是「Claude 尚未登录」对「ACP Design 尚未登录」—— 仍然不相等。
     * 判据要钉在 S04 说的**是不是产品那句**:`ACP Design`(无空格)是 agent
     * 标签,`ACP Design`(有空格)才是产品名。
     */
    expect(s04).not.toContain('ACP Design');
  });
  ...
  it('Antigravity 认证失败保留 S02 文案,固定入口切换到 Cloud', () => {
    ...
    expect(within(card).getAllByRole('button').map((button) => button.textContent?.trim()))
      .toEqual(['联系我们', '导出日志', '切换到 ACP Design Cloud']);
    expect(within(card).queryByRole('button', { name: '在终端中登录' })).toBeNull();
    fireEvent.click(within(card).getByRole('button', { name: '切换到 ACP Design Cloud' }));
