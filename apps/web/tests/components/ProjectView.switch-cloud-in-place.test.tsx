...
describe('ProjectView OPEND-3205 in-project Cloud switch', () => {
  ...
  it.each([
    ...
    const switchButton = within(card).getByRole('button', { name: '切换到 ACP Design Cloud' });
    ...
    expect(await view.findByText('已切换到 ACP Design Cloud，请重新发送任务。')).toBeVisible();
  ...
  it.each(['pending double click', 'save failure', 'authority changed', 'view unmounted'] as const)(
    'keeps the Cloud success receipt honest when %s', async (scenario) => {
      ...
      const action = within(card).getByRole('button', { name: '切换到 ACP Design Cloud' });
      try {
        await act(async () => {
        ...
        expect(arm).not.toHaveBeenCalled();
        expect(streamViaDaemon).not.toHaveBeenCalled();
        expect(view.queryByText('已切换到 ACP Design Cloud，请重新发送任务。')).toBeNull();
        ...
        if (scenario === 'pending double click') {
          ...
          expect(document.body.textContent).not.toContain('已切换到 ACP Design Cloud，请重新发送任务。');
        }
  ...
  it.each([
    ...
    const host = side ? await view.findByTestId('side-chat-recovery-host') : view.container;
    await within(host).findByText(failure.content);
    const action = within(within(host).getByTestId('chat-run-error-card')).getByRole('button', { name: '切换到 ACP Design Cloud' });
    if (readOnly) {
