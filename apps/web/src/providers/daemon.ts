...
export interface DaemonStreamOptions {
   ...
   *  `created` is the POST /api/runs body when this hop came from create
   *  (carries designGenerationSurfaces); reattach successor hops omit it. */
  onRunCreated?: (
    runId: string,
    strategyTask?: StrategyTaskProjectionV2,
    created?: ChatRunCreateResponse,
  ) => void;
...
export async function streamViaDaemon({
  ...
  try {
    ...
    onRunCreated?.(runId, created.strategyTask, created);
