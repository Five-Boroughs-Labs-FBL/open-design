...
type RunFailureUiExtra = RunFailureDaemonVerdict & {
  promoteAmr?: boolean;
  verdict?: RunFailureDaemonVerdict | null;
};

function runFailureVerdictFromExtra(
  extra: RunFailureUiExtra | RunFailureDaemonVerdict | null | undefined,
): RunFailureDaemonVerdict | null | undefined {
  if (!extra || typeof extra !== 'object') return extra;
  if ('promoteAmr' in extra || 'verdict' in extra) {
    const options = extra as RunFailureUiExtra;
    return options.verdict ?? (
      options.retryable !== undefined || options.failureAction !== undefined
        ? { retryable: options.retryable, failureAction: options.failureAction }
        : undefined
    );
  }
  return extra;
...
export function resolveRunFailureUi(
  ...
  rawMessage?: string | null,
  extra?: RunFailureUiExtra | RunFailureDaemonVerdict | null,
): RunFailureUi {
  const verdict = runFailureVerdictFromExtra(extra);
  ...
  const promoted = runsOnALocalAgent(agentId)
    ? withCloudSwitchCta(ui)
    : withoutCloudSelfPromotion(ui);
  if (extra && typeof extra === 'object' && 'promoteAmr' in extra && extra.promoteAmr === false) {
    return { ...promoted, cloudSwitchCta: false };
  }
  return promoted;
}
