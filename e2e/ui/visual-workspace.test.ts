...
test('[P1] captures the topbar ACP Design model picker with no account surface', async ({ page }) => {
  ...
  // with ACP Design installed, so a landed response proves the signed-in
  // render happened.
  ...
  // nothing else. The agent grid (which is where the ACP Design-first ordering
  ...
  // upgrade card is captured by visual-settings.test.ts's "settings ACP Design
...
test('[P1] Avatar menu stays a model picker for a signed-in ACP Design account', async ({ page }) => {
  test.setTimeout(60_000);
  ...
  const requestsBeforeOpen = velaStatusRequests;
  const menu = await prepareVisualAvatarMenu(page);
  // AvatarMenu refetches the login status on open whenever ACP Design is
  // installed, so a landed response means the signed-in render has happened.
  await expect.poll(() => velaStatusRequests).toBeGreaterThan(requestsBeforeOpen);

  // 4e3161751 (#6156) deleted the ACP Design account row from this popover —
  ...
  // visual-settings.test.ts's "settings ACP Design account balance" case, and
  ...
  // What it does render for a signed-in ACP Design runtime: that runtime's
