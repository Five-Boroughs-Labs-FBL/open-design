...
test('[P0] ACP handshake refusal is actionable, persists, and does not auto-retry', async ({ page }) => {
  ...
  await expect(card.getByTestId('chat-run-error-description')).toHaveText(
    'ACP Design does not currently support this agent version. Please switch to a supported version and try again.',
  );
  await expect(card).not.toContainText(rawError);
...
test('[P1] real daemon classifies a Claude prompt-too-long result and preserves it across reload', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Switch to ACP Design Cloud', exact: true })).toHaveCount(1);
...
test('[P1] plugin authoring produces a generated-plugin scaffold with action cards', async ({ page }) => {
  ...
  await expect(page.getByTestId('home-hero-input')).toHaveText(/Create an ACP Design plugin for:/);
