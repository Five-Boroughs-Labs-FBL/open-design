...
test('[P0] @critical onboarding lets AMR Cloud sign in and complete setup after the login poll succeeds', async ({ page }) => {
  ...
  // Signed-out cloud landing: the primary button reads "Sign in to ACP Design
  ...
  await expect(primary).toHaveText(/Sign in to ACP Design|登录 ACP Design/i);
  ...
  await continueWithModelSource(page, /ACP Design Hosted/i);
...
test('[P0] signed-out onboarding can open Local CLI setup without Cloud authorization', async ({ page }) => {
  ...
  await expect(cloudPrimaryButton(page)).toHaveText(/Sign in to ACP Design|登录 ACP Design/i);
...
function connectLandingHeading(page: Page): Locator {
  return page.getByRole('heading', { name: /Sign in to ACP Design|登录 ACP Design/i });
}
