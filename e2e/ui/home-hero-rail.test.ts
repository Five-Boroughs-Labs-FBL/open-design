...
const HOME_PLUGINS = [
  ...
  },
  {
    ...
    manifest: {
      ...
      title: 'Live Artifact',
      version: '0.1.0',
      description: 'Create refreshable, auditable ACP Design artifacts.',
      od: {
        kind: 'scenario',
        taskKind: 'new-generation',
        mode: 'prototype',
...
async function waitForLoadingToClear(page: Page) {
  await page.getByText('Loading ACP Design…').waitFor({ state: 'hidden', timeout: 15_000 });
}
...
test('[P1] selecting another example updates the composer input', async ({ page }) => {
  ...
  await expect(input).toHaveText('Create refreshable, auditable ACP Design artifacts.');
});
